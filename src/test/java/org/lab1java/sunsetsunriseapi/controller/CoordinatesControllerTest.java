...
class CoordinatesControllerTest {
    ...
    void testGetCoordinatesInfo() throws JsonProcessingException {
        ...
        when(coordinatesService.getCoordinatesInfo(eq(userId), any(RequestDto.class))).thenReturn(responseDto);

        ResponseEntity<ResponseDto> response = coordinatesController.getCoordinatesInfo(userId, latitude, longitude, date);
    ...
    void testGetUsersFromCoordinates() {
        ...
        Page<User> userPage = new PageImpl<>(userList, PageRequest.of(pageNumber, pageSize), userList.size());
        when(coordinatesService.getUsersFromCoordinates(id, pageNumber, pageSize)).thenReturn(userPage);

        ResponseEntity<Page<User>> response = coordinatesController.getUsersFromCoordinates(id, pageNumber, pageSize);
    ...
    void testGetAllCoordinateInfo() {
        ...
        coordinatesDtoList.add(new CoordinatesDto(17.73562, 9.323843, LocalDate.parse("2024-03-20"), LocalTime.of(6, 25, 13),
                LocalTime.of(18, 34, 49), "Africa/Niamey", "Niger", "Niamey"));
        coordinatesDtoList.add(new CoordinatesDto(51.508530, -0.125740, LocalDate.parse("2024-03-20"), LocalTime.of(5, 55, 24),
                LocalTime.of(18, 19, 1), "Europe/London", "England", "London"));
        Page<CoordinatesDto> coordinatesDtoPage = new PageImpl<>(coordinatesDtoList, PageRequest.of(pageNumber, pageSize), coordinatesDtoList.size());
        when(coordinatesService.getAllCoordinatesInfo(pageNumber, pageSize)).thenReturn(coordinatesDtoPage);

        ResponseEntity<Page<CoordinatesDto>> response = coordinatesController.getAllCoordinateInfo(pageNumber, pageSize);
    ...
    void testUpdateCoordinatesInfo() {
        ...
        CoordinatesDto updateDto = new CoordinatesDto(17.73562, 9.323843, LocalDate.parse("2024-03-20"), LocalTime.of(6, 25, 13),
                LocalTime.of(18, 34, 49), "Africa/Niamey", "Niger", "Niamey");
        Coordinates updatedCoordinates = new Coordinates(latitude, longitude, date, sunrise, sunset, timeZone, city);
        when(coordinatesService.updateCoordinatesInfo(id, updateDto)).thenReturn(updatedCoordinates);

        ResponseEntity<Coordinates> response = coordinatesController.updateCoordinatesInfo(id, updateDto);
