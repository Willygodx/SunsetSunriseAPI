...
class CountryControllerTest {
  ...
  void testGetCoordinatesInfoForCountry() {
    ...
    when(countryService.getCoordinatesInfoForCountry(countryName, pageNumber, pageSize)).thenReturn(
        mockPage);

    ResponseEntity<Page<Coordinates>> response =
        countryController.getCoordinatesInfoForCountry(countryName, pageNumber, pageSize);

    assertEquals(HttpStatus.OK, response.getStatusCode());
    assertEquals(mockPage, response.getBody());
    verify(countryService, Mockito.times(1)).getCoordinatesInfoForCountry(countryName, pageNumber,
        pageSize);
  }
  ...
  void testGetAllCountries() {
    ...
    ResponseEntity<Page<Country>> response =
        countryController.getAllCountries(pageNumber, pageSize);
